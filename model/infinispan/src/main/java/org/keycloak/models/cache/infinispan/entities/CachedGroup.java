...
public class CachedGroup extends AbstractRevisioned implements InRealm {
    ...
    private final LazyLoader<GroupModel, Set<String>> childGroupReference;
    private final LazyLoader<GroupModel, Set<String>> parentGroupReference;
    ...
    public CachedGroup(Long revision, RealmModel realm, GroupModel group) {
        ...
        this.childGroupReference = new DefaultLazyLoader<>(source -> source.getChildGroupsReference().stream().map(GroupModel::getId).collect(Collectors.toSet()), Collections::emptySet);
        this.parentGroupReference = new DefaultLazyLoader<>(source -> source.getParentGroupsReference().stream().map(GroupModel::getId).collect(Collectors.toSet()), Collections::emptySet);
    }
    ...
    public Set<String> getChildGroupsReference(Supplier<GroupModel> group) { return childGroupReference.get(group); }

    public Set<String> getParentGroupsReference(Supplier<GroupModel> group) { return parentGroupReference.get(group); }
}
